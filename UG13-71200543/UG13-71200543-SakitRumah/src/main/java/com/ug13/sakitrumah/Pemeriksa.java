...
public class Pemeriksa {
    ...
    public void memeriksa(Pengunjung pengunjung, Jadwal jadwal){
        if (jadwal.getStatusScreening()){
            ...
            System.out.println("maaf, ANDA HARUS MELAKUKAN PENDAFTARAN TERLEBIH DAHULU");
        }
    }
    public void cekStatus(Pengunjung p){
        if (p.getStatus()){
            System.out.println("SELAMAT PASIEN ANDA SUDAH SEMBUH");
        }else{
            System.out.println("MOHON MAAF PASIEN ANDA MASIH SAKIT");
        }
    }
}